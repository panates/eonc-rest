    ...
    handle(req, res) {
        ...
        if (layer && layer.handler) {
            try {
                ...
                req.args = this._types.deserialize(layer.method, query);
                // call handler
            ...
            res.statusCode = 400;
            res.statusMessage = 'Invalid request. This endpoint can`t handle HTTP ' + req.method + ' method';
            res.end();
        }
        if (res.statusCode >= 400 && res.statusCode < 500)
            debug('W: %s | %s - %s', this.fullRoute, res.statusCode, res.statusMessage);
        else if (res.statusCode >= 500 && res.statusCode < 600)
            debug('E: %s | %s - %s', this.fullRoute, res.statusCode, res.statusMessage);
    }
