      ...
      if (layer && layer.handler) {
        ...
        if (o && (o instanceof Promise ||
          ...
          o.catch(e => {
            ...
            err.request = req;
            err.route = self.route;
            err.fullRoute = self.fullRoute;
            res.end(err);
          });
        }
      } else {
        const err = new errors.HttpError(400, 'Invalid request. This endpoint can`t handle HTTP ' +
            req.method + ' method');
        err.route = self.route;
        err.fullRoute = self.fullRoute;
        res.end(err);
