...
class Schema {
    ...
    deserialize(typeDef, input) {
        ...
        if (util.isObject(typeDef)) {
            ...
            if (!ti.optional && (out === undefined || out === null || out === ''))
    ...
    static parseTypeDef(name, typeDef) {
        ...
        if (out.ns) {
            ...
            if (!(schema && schema.get(out.type)))
                throw new TypeNotFoundError(`Type not found "${out.ns}:${out.type}"`);
        }
        //console.log(out);
        return out;
    }
    ...
    static parseTypeObject(name, typeObj) {
        ...
        if (typeObj.minValue !== undefined)
            out.minValue = typeObj.minValue;
        if (typeObj.maxValue !== undefined)
            out.maxValue = typeObj.maxValue;
    ...
    static parseTypeString(name, str) {
        ...
        // Regex pattern
        if (m[9])
            out.pattern = new RegExp(m[9].substring(1, m[9].length - 1));
