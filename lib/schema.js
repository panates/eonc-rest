...
const parseDate = helpers.parseDate;
...
class Schema {
    ...
    static get(namespace) {
        ...
        if (m && m.length >= 3) {
            let schema = Schema.get(m[1]);
            if (!schema)
                throw new SchemaError(`Schema not found for ns "${m[1]}"`);
            return schema.get(a[2]);
        }
        return gSchemasNS[namespace.toLowerCase()] || gSchemas[namespace.toLowerCase()];
    }
...
class SchemaItem {
    ...
    _parseString(str) {
        if (!str)
            throw new SchemaError('Empty type definition for "' + this.name + '"');
    ...
    validate() {
        ...
        if (this.minValue !== undefined || this.maxValue !== undefined) {
            ...
            if (typ.match(/^(ointeger|long)$/)) {
                this.minValue = parseInt(this.minValue, 10) || undefined;
                this.maxValue = parseInt(this.maxValue, 10) || undefined;
            } else if (typ.match(/^(number|double)$/)) {
                this.minValue = parseFloat(this.minValue) || undefined;
                this.maxValue = parseFloat(this.maxValue) || undefined;
            } else if (typ === "date") {
                this.minValue = parseDate(this.minValue) || undefined;
                this.maxValue = parseDate(this.maxValue) || undefined;
            }
    ...
    deserialize(input) {
        ...
        switch (merged.type) {
            ...
            case "double": {
                if (!String(input).match(numberPattern))
                    throw new errors.InvalidRequestError(`Invalid ${merged.type} value "${input}"`);
            ...
            case "object": {
                ...
                if (merged.items) {
                    out = {};
                    Object.getOwnPropertyNames(merged.items).forEach(name => {
                        let ti = merged.items[name],
                            val = input[ti.name];
        ...
        if (merged.onvalidate)
            out = merged.onvalidate(merged, out);

        if (merged.pattern && !String(input).match(merged.pattern))
            throw new Error(`Input value does not match given pattern`);
        ...
        switch (merged.type) {
            ...
            case "integer":
            case "date": {
                if ((merged.minValue !== undefined && out < merged.minValue) ||
                    (merged.maxValue !== undefined && out > merged.maxValue)) {
                    throw new Error(this.name + ' have to be a ' + merged.type + ' value' +
                        (merged.minValue !== undefined && merged.maxValue !== undefined ?
                                ' between ' + merged.minValue + ' and ' + merged.maxValue :
                                (
                                    (merged.minValue !== undefined ? ' >= ' + merged.minValue : '') +
                                    (merged.maxValue !== undefined ? ' <= ' + merged.maxValue : '')
                                )
                        )
                    );
                }
        ...
        if (!merged.optional && (out === undefined || out === null || out === ''))
    ...
    _merge() {
        ...
        while (typ) {
            ...
            let fields = ["pattern", "minOccurs", "maxOccurs", "minValue", "maxValue", "optional", "onvalidate"], field;
