...
class Schema {
    ...
    static get(namespace) {
        ...
        if (m && m.length >= 3) {
            ...
            if (!schema)
                throw new SchemaNotFoundError(`Schema not found for ns "${m[1]}"`);
            return schema.get(m[2]);
        }
...
class SchemaItem {
    ...
    _deserialize(typeObj, input) {
        ...
        if (typeObj.pattern && !String(input).match(typeObj.pattern))
        ...
        switch (typeObj.type) {
            ...
            case "date": {
                if ((typeObj.minValue !== undefined && out < typeObj.minValue) ||
                    (typeObj.maxValue !== undefined && out > typeObj.maxValue)) {
                    throw new DeserializeError(this.name + ' have to be a ' + typeObj.type + ' value' +
                        (typeObj.minValue !== undefined && typeObj.maxValue !== undefined ?
                                ' between ' + typeObj.minValue + ' and ' + typeObj.maxValue :
                                (
                                    (typeObj.minValue !== undefined ? ' >= ' + typeObj.minValue : '') +
                                    (typeObj.maxValue !== undefined ? ' <= ' + typeObj.maxValue : '')
                                )
                        )
                    );
                }
        ...
        if (!typeObj.optional && (out === undefined || out === null || out === ''))
            throw new DeserializeError(`Parameter "${ti.name}" is required`);
