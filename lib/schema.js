...
const parseDate = function(val) {
  ...
  if (!d)
    throw new DeserializeError(`Indalid date value "val"`);
  return d;
};
...
class Schema {
  ...
  define(name, typeDef) {
    ...
    if (typeof name === 'object') {
      Object.getOwnPropertyNames(name).forEach(n => {
        self.define(n, name[n]);
      });
      return;
    }
   ...
   * @public
   */
  deserialize(typeName, input, req) {
    const item = this.get(typeName);
    if (!item)
      return Schema.deserialize(typeName, input, req);
    return item.deserialize(input, req);
  }

  static deserialize(typeName, input, req) {
    typeName = String(typeName);

    if (typeName.includes(':')) {
      const m = typeName.match(/([\w]*):([\s\S]*)/);
      if (m)
        return Schema.get(m[1]).deserialize(m[2], input, req);
    }
    ...
    switch (typeName.toLowerCase()) {
      ...
      case 'object':
        return typeof input !== 'object' ? jsonic(input) : input;

    }
    throw new TypeNotFoundError(`Unknown type "${typeName}"`);
  }
...
class SchemaItem {
    ...
    if (this.minOccurs !== undefined || this.maxOccurs !== undefined) {
      ...
      if (Array.isArray(input)) {
        for (let i = 0; i < input.length; i++) {
          out.push(this._deserialize(merged, input[i], req));
        }
      } else out.push(this._deserialize(merged, input, req));
    } else
      out = this._deserialize(merged, input, req);
   ...
   * @public
   */
  _deserialize(typeObj, input, req) {

    let out;
    if (input) {
      if (internalTypes.includes(typeObj.type))
        out = Schema.deserialize(typeObj.type, input, req);
      if (typeObj.type === 'object' && typeObj.items) {
        const inp = out;
        out = {};
        Object.getOwnPropertyNames(typeObj.items).forEach(name => {
          const ti = typeObj.items[name];
          const val = inp[ti.name];
          out[name] = ti.deserialize(val, req);
        });
      }

      if (typeObj.onValidate)
        out = typeObj.onValidate(out, typeObj, req);
    }
