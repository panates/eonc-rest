...
const proto = {
  ...
  use: function use(route, fn) {
    ...
    function patchResponse(res) {
      if (res.end === patchResponse) return;
      let ended;
      ...
      res.write = function(chunk, encoding, callback) {
        if (ended) {
          if (process.env.DEBUG)
            debug('Warning!. Write after end', new Error().stack);
          return;
        }
        if (chunk instanceof Error) {
          const err = chunk;
          err.fullRoute = fullRoute;
          if (!res.headersSent)
            res.statusCode =
                (err instanceof errors.HttpError ? err.code : 500) || 400;
          oldWrite.call(res, String(err), callback);
        } else
        ...
        lhandle = function(req, res, next) {
          ...
          try {
            ...
            if (o && (o instanceof Promise ||
              ...
              o.catch(e => {
                res.end(e instanceof Error ? e : new Error(e));
              });
            }
          } catch (e) {
            res.end(e instanceof Error ? e : new Error(e));
