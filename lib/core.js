...
function createRestHandler(cfg) {
  ...
  restHandler.route = '/';
  restHandler.stack = [];
  restHandler.writeObject = cfg ? cfg.writeObject : undefined;
  return restHandler;
}
...
const proto = {
  ...
  use: function use(route, fn) {
    ...
    function patchResponse(res) {
      ...
      let ignoreWriteObject;
      res.write = function(chunk, encoding, callback) {
        if (!ignoreWriteObject) {
          ignoreWriteObject = true;
          try {
            if (self._writeObject(res, chunk, encoding, callback)) return;
          } finally {
            ignoreWriteObject = false;
          }
        }
    ...
    if (lhandle instanceof constructor &&
      ...
      lhandle = function(req, res, next) {
        req.route = middleware.route;
        req.fullRoute = middleware.fullRoute;
        patchResponse(res);
