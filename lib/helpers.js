...
exports = module.exports = {
    ...
    parseDate: function (val) {
        ...
        if (val.indexOf(".") === 2) { // dd.mm.yyyy format
            let m = String(val).match(dayFirstDatePattern);
            if (m) {
                if (m[8]) {
                    let utcdate = Date.UTC(parseInt(m[3]), parseInt(m[2]) - 1, parseInt(m[1]),
                        parseInt(m[4]) || 0, parseInt(m[5]) || 0, parseInt(m[6]) || 0, parseInt(m[7]) || 0);
                    if (m[10] && m[11]) {
                        let offsetMinutes = parseInt(m[10]) * 60 + parseInt(m[11]);
            ...
            let m = String(val).match(monthFirstDatePattern);
            if (m) {
                if (m[8]) {
                    let utcdate = Date.UTC(parseInt(m[3]), parseInt(m[1]) - 1, parseInt(m[2]),
                        parseInt(m[4]) || 0, parseInt(m[5]) || 0, parseInt(m[6]) || 0, parseInt(m[7]) || 0);
                    if (m[10] && m[11]) {
                        let offsetMinutes = parseInt(m[10]) * 60 + parseInt(m[11]);
                        utcdate += (m[9] === '+' ? -1 : +1) * offsetMinutes * 60000;
            ...
            let m = String(val).match(yearFirstDatePattern);
            if (m) {
                if (m[8]) {
                    let utcdate = Date.UTC(parseInt(m[1]), parseInt(m[2]) - 1, parseInt(m[3]),
                        parseInt(m[4]) || 0, parseInt(m[5]) || 0, parseInt(m[6]) || 0, parseInt(m[7]) || 0);
                    if (m[10] && m[11]) {
                        let offsetMinutes = parseInt(m[10]) * 60 + parseInt(m[11]);
                        utcdate += (m[9] === '+' ? -1 : +1) * offsetMinutes * 60000;
